...
export function ForgeMobileNav({ open, onOpenChange }: ForgeMobileNavProps) {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/40 backdrop-blur-xs z-[100] transition-opacity" />
        <Dialog.Content 
          className={cn(
            "fixed inset-y-0 left-0 z-[101] w-[280px]",
            "bg-[var(--color-surface-raised)] rounded-r-xl shadow-lg border-r border-[var(--color-border)]",
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b border-[var(--color-border)]">
            <Link href="/" className="flex items-center gap-3" onClick={() => onOpenChange(false)}>
              <img
                src="/logo-icon.png"
                alt="ExamForge Logo"
                className="w-9 h-9 rounded-lg object-cover shadow-xs border border-[var(--color-border)]"
              />
            ...
            <Dialog.Close asChild>
              <button className="p-2 -mr-2 rounded-full text-[var(--color-ink-secondary)] hover:bg-[var(--color-surface-sunken)] transition-colors">
                <X className="w-5 h-5" />
              </button>
            </Dialog.Close>
          </div>
                  ...
                  {section.items.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          "flex items-center justify-between px-4 py-2.5 min-h-[44px] rounded-lg text-sm font-medium transition-all",
                          isActive
                            ? "bg-[var(--color-accent-surface)] text-[var(--color-accent)] font-semibold shadow-xs"
                            : "text-[var(--color-ink)] hover:text-[var(--color-ink)] hover:bg-[var(--color-surface-sunken)]"
                        )}
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <Icon className={cn("w-4 h-4 shrink-0", isActive ? "text-[var(--color-accent)]" : "text-[var(--color-ink-secondary)]")} />
          ...
          <div className="p-4 border-t border-[var(--color-border)] bg-[var(--color-surface-sunken)] space-y-3">
            <div className="text-xs font-semibold text-[var(--color-ink-secondary)] uppercase tracking-wider">
              Active Persona
            </div>
            <div className="relative">
              <select
                ...
                className={cn(
                  ...
                  "text-[var(--color-ink)] cursor-pointer truncate",
              ...
              <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--color-ink-secondary)] pointer-events-none" />
            </div>

            <button
              onClick={handleLogout}
              className="w-full flex items-center justify-center gap-2 py-2 min-h-[44px] rounded-lg text-sm font-medium text-[var(--color-danger)] hover:bg-[var(--color-danger-surface)] transition-colors cursor-pointer"
            >
